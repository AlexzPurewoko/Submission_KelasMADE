...
class ApiRepository {
    ...
    suspend fun doReqandRetResponse(url: String, tag: String, priority: Priority): ReturnedResults? = withContext(Dispatchers.Default){
    ...
    fun doReqAndRetResponseAsync(url: String, tag: String, priority: Priority) : Deferred<ReturnedResults?> = GlobalScope.async {
        var hasFinished = false
        var returnedResults: ReturnedResults? = null
        AndroidNetworking.get(url)
            .setTag(tag)
